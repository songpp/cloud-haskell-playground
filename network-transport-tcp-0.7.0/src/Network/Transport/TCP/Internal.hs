  ...
  N.SockAddrInet port host -> do
    ...
    case (mResolvedHost, mResolvedPort) of
      (Just resolvedHost, Nothing) -> do
        (resolvedNumericHost, _) <- N.getNameInfo [N.NI_NUMERICHOST] True False sockAddr
        case resolvedNumericHost of 
          Just numericHost -> return $ Just (numericHost, resolvedHost, show port)
